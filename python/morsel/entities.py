...
class Triangle:
    ...
    def get_sample_vector(self):
        while True:
            ...
            random_add = Vector.sum(Vector.mult_const(self.r1, b1),
                                    Vector.mult_const(self.r2, b2))
            vec = Vector.sum(self.origin, random_add)

            if self.inside_triangle(vec, self.origin,
                                    Vector.sum(self.origin, self.r1),
                                    Vector.sum(self.origin, self.r2)):
                return vec
